  ...
  return (
    <div className="p-4 overflow-auto min-h-[calc(100vh-4rem)] flex flex-col gap-y-4">
      ...
      <div className="grid grid-cols-3 gap-4 rounded-md shadow-[inset_0_0px_10px_rgba(0,0,0,0.10)] p-4">
        {gitComponents &&
          gitComponents.length > 0 &&
          gitComponents.map((route: any) => (
            <Link
              key={route.name}
              to={route.href}
              className={`flex justify-center flex-col gap-y-1 bg-gray-100 p-4 text-sm font-medium rounded-sm transition-all duration-200 text-gray-700 hover:bg-primary/20 hover:text-primary shadow-[0px_0px_10px_rgba(0,0,0,0.20)] h-[52px]`}
            >
